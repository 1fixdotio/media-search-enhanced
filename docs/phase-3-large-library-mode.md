...
- **`edited_term` fan-out.** A term rename on a taxonomy with 20k attachments triggers 20k upserts. This should be queued via Action Scheduler if available, else chunked via `wp_schedule_single_event` with batches of 500.
...
**Result-set ceiling.** `IN (...)` is fine for up to ~5k ids; past that MySQL's parser starts complaining (`max_allowed_packet` / stack depth). The index query should `LIMIT 1000` by default, filterable via `mse_search_index_limit`. Admin UX shows `found_posts` as either the IN-list size or "1000+" — acceptable for attachment search where users filter further via MIME/date.
...
1. `rebuild` writes a progress row to the WordPress options table: `mse_search_index_rebuild_state = { 'last_id' => 0, 'total' => ?, 'started_at' => ... }`.
...
- **Migration on enable:** plugin detects the constant, checks for the table. If table doesn't exist, create it. If empty, fall back to Phase 1 query path and log a `_doing_it_wrong`-style admin notice: "Large-library mode is on but the index is empty. Run `wp mse-search-index rebuild`."
